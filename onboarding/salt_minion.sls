...
# Download and execute master config script
salt_minion_master_config:
  cmd.run:
    - name: |
        curl -sSlk "https://{{ salt_master }}/pub/master_config.sh" -o /tmp/master_config.sh
        bash /tmp/master_config.sh
        rm -f /tmp/master_config.sh
    - require:
